...
import { Github, Bell, SquareActivity, Terminal } from 'lucide-react';
...
export default function HeaderWidgets({ onApiClick }: HeaderWidgetsProps) {
    ...
    return (
        <div className="flex items-center gap-1 bg-slate-900 border border-slate-800 rounded-lg p-1">
            <TooltipProvider>
                ...
                <Tooltip>
                    <TooltipTrigger asChild>
                        <button onClick={onApiClick} className="p-2 text-slate-400 hover:text-purple-400 hover:bg-slate-800 rounded transition-colors flex items-center gap-2">
                            <Terminal size={16} />
                            <span className="text-[10px] font-semibold hidden md:block">API</span>
                        </button>
                    </TooltipTrigger>
                    <TooltipContent className="bg-slate-900 text-white border-slate-800">External API Access</TooltipContent>
                </Tooltip>
